    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-black text-white">Garanti Sorgulama & Destek Talebi</h1>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
        <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            type="submit"
            className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs px-8 py-3 rounded-xl shadow-lg transition flex items-center justify-center gap-2"
          >
      ...
      {hasSearched && queriedRecord && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-6 animate-in fade-in">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-4">
            <div>
              <span className="text-[10px] text-emerald-400 font-bold uppercase">Sorgulanan Cihaz</span>
              <h2 className="text-lg font-black text-white">{queriedRecord.productName}</h2>
              <span className="font-mono text-xs text-sky-400">Seri No: {queriedRecord.serialNumber}</span>
            </div>

            <div className="flex items-center gap-2">
              <span className="bg-emerald-950 text-emerald-400 border border-emerald-800 font-bold text-xs px-3.5 py-1.5 rounded-xl flex items-center gap-1.5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
            <div className="bg-slate-950 p-3.5 rounded-xl border border-slate-800">
              ...
              <span className="font-bold text-white mt-1 block">{queriedRecord.brand}</span>
              <span className="text-[10px] text-slate-400">{queriedRecord.model}</span>
            </div>

            <div className="bg-slate-950 p-3.5 rounded-xl border border-slate-800">
              <span className="text-[10px] text-slate-500 uppercase font-bold block">Montaj Tarihi</span>
              <span className="font-bold text-white mt-1 block">{queriedRecord.installDate}</span>
            </div>

            <div className="bg-slate-950 p-3.5 rounded-xl border border-slate-800">
              <span className="text-[10px] text-slate-500 uppercase font-bold block">Garanti Bitişi</span>
              <span className="font-bold text-emerald-400 mt-1 block">{queriedRecord.warrantyEndDate}</span>
            </div>

            <div className="bg-slate-950 p-3.5 rounded-xl border border-slate-800">
              <span className="text-[10px] text-slate-500 uppercase font-bold block">Sorumlu Bayi</span>
              <span className="font-bold text-white mt-1 block line-clamp-1">{queriedRecord.dealerName}</span>
            </div>
          </div>
        ...
        {warrantyClaims.length === 0 ? (
          ...
          <div className="space-y-3">
            {warrantyClaims.map((claim) => (
              <div key={claim.id} className="bg-slate-950 border border-slate-800 p-4 rounded-xl text-xs space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="font-mono font-bold text-sky-400">#{claim.claimNumber}</span>
                ...
                <div className="text-slate-200 font-bold">{claim.productName}</div>
                <div className="font-mono text-[11px] text-slate-400">Seri No: {claim.serialNumber}</div>
                <p className="text-slate-300 bg-slate-900 p-2.5 rounded-lg text-[11px]">
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h3 className="text-base font-bold text-white">Garanti / Servis Talebi Oluştur</h3>
            ...
            <form onSubmit={handleCreateClaim} className="space-y-3 text-xs">
              <div>
                <label className="block text-slate-400 font-semibold mb-1">Cihaz Seri Numarası:</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-slate-400 font-semibold mb-1">Ürün / Model Adı:</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-slate-400 font-semibold mb-1">Arıza / Sorun Açıklaması:</label>
                <textarea
                  rows={4}
                  required
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button
                  type="submit"
                  className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-2.5 rounded-xl shadow-lg transition"
                >
                  Talebi Gönder
                </button>
              </div>

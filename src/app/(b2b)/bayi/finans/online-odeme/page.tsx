...
export default function OnlinePaymentPage() {
  ...
  const handleComplete3DSecure = (e: React.FormEvent) => {
    e.preventDefault();
    ...
    setTimeout(() => {
      ...
      const refCode = `SANPOS-${Math.floor(5000 + Math.random() * 4999)}`;
      const now = new Date();
      const dateStr = `${String(now.getDate()).padStart(2, '0')}.${String(now.getMonth() + 1).padStart(2, '0')}.${now.getFullYear()} ${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`;

      const createdSlip = addPosSlip({
        date: dateStr,
        ...
        amount: currentInstallmentOption?.totalAmount || numericAmount,
        status: 'Başarılı',

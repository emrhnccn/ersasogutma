...
export function InvoiceDetailModal({ invoice, onClose }: InvoiceDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-4xl w-full overflow-hidden shadow-2xl animate-in zoom-in-95 flex flex-col max-h-[90vh]">
        ...
        <div className="p-4 sm:p-5 border-b border-slate-800 bg-slate-950/80 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-black text-white">Fatura Detayı</h3>
                <span className="font-mono font-bold text-sky-400 bg-sky-950 px-2 py-0.5 rounded border border-sky-800 text-xs">
                  {invoice.invoiceNumber}
                </span>
              </div>
              <div className="text-[11px] text-slate-400">
                E-Arşiv Belge No: <span className="font-mono text-slate-300">{invoice.eArchiveId || '-'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handlePrint}
              className="px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white text-xs font-semibold border border-slate-700 flex items-center gap-1.5 transition"
            >
        ...
        <div className="p-6 overflow-y-auto space-y-6 text-xs">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-1">
              ...
              <div className="text-slate-400">Vergi Dairesi: Darıca V.D. / VKN: 3340592817</div>
              <div className="text-slate-400">Adres: Kazım Karabekir Mah. İstasyon Cad. No:84 Darıca / Kocaeli</div>
              <div className="text-slate-400">Tel: 0262 653 41 00 • Mersis: 0334059281700018</div>
            </div>
            ...
            <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-1">
              ...
              <div className="text-slate-400">Vergi Dairesi: {invoice.taxOffice} / VKN: {invoice.taxNumber}</div>
              <div className="text-slate-400">Adres: {invoice.address}</div>
              ...
              <tbody className="divide-y divide-slate-800 text-slate-200">
                {invoice.items.map((item, idx) => (
                  <tr key={idx} className="hover:bg-slate-800/40">
                    ...
                    <td className="py-2.5 px-3 font-mono font-bold text-sky-400">{item.code}</td>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-950 p-4 rounded-xl border border-slate-800">
            ...
            <div className="w-full sm:w-72 space-y-1.5 text-xs text-slate-300">
              <div className="flex justify-between">
                <span>Mal Hizmet Toplamı:</span>
                <span className="font-mono font-bold text-white">{formatCurrency(invoice.subtotal)}</span>
              </div>
              <div className="flex justify-between">
                <span>KDV Matrahı:</span>
                <span className="font-mono text-slate-300">{formatCurrency(invoice.vatMatrah)}</span>
              </div>
              <div className="flex justify-between">
                <span>Hesaplanan KDV:</span>
                <span className="font-mono text-slate-300">{formatCurrency(invoice.vatTotal)}</span>
              </div>
              <div className="pt-2 border-t border-slate-700 flex justify-between font-black text-sm text-emerald-400">
